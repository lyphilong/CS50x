...
def shortest_path(source, target):
    ...
    while True:
        ...
        for action, state in neighbors_for_person(node_current.state):
            if (not frontier.contains_state(state)) and (state not in explored): #Nếu như node kề chưa được duyệt và không nằm trong frotier
                child = Node(state=state, parent=node_current, action=action) #Node của nó bằng node hiện đang xét, cha là node trước, action là phim
                frontier.add(child)
